...
export const SubscriptionInvoicePrint = ({ subscription, school, autoPrint = true }) => {
  useEffect(() => {
    if (subscription && autoPrint) {
      const timer = setTimeout(() => {
        window.print();
      }, 200);
      return () => clearTimeout(timer);
    }
  }, [subscription, autoPrint]);
  ...
  return (
    <div
      ...
      className="hidden print:block bg-white p-8 sm:p-10 rounded-none text-slate-900 font-sans space-y-8 print:w-full print:m-0 print:p-0"
    >
      {/* Header Block */}
      <div className="flex flex-row justify-between items-start gap-6 border-b border-slate-300 pb-6">
      ...
      <div className="grid grid-cols-2 gap-6 bg-slate-50/80 p-5 rounded-xl border border-slate-200 text-xs">
        <div>
      ...
      <div className="flex flex-row justify-between items-start gap-6 pt-2">
      ...
      <div className="flex flex-row justify-between items-end gap-6 pt-6 border-t border-slate-200 text-xs">
