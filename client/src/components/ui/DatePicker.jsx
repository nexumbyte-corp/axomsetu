...
export const DatePicker = ({
  ...
  return (
    <div className={`w-full relative ${className}`} ref={containerRef}>
      {cleanedLabel && (
        <label htmlFor={inputId} className={`block font-semibold uppercase tracking-wider text-slate-700 ${isSm ? 'text-[10px] mb-1 font-bold' : 'text-xs mb-1.5'} ${labelClassName}`}>
          {cleanedLabel}
      ...
      <div className="relative rounded-lg shadow-2xs">
        ...
        <button
          ...
          className={`absolute inset-y-0 left-0 flex items-center cursor-pointer text-indigo-600 hover:text-indigo-800 transition-colors disabled:cursor-not-allowed ${isSm ? 'pl-2.5' : 'pl-3'}`}
        >
          <CalendarIcon className={isSm ? 'w-3.5 h-3.5 shrink-0' : 'w-4 h-4 shrink-0'} />
        </button>
        ...
        <input
          ...
          className={`w-full rounded-lg border text-xs font-semibold font-mono transition-all duration-150 focus:outline-hidden focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-600 disabled:cursor-not-allowed ${
            isSm ? 'pl-8 pr-7 py-1.5' : 'pl-9 pr-8 py-2'
          } bg-white text-slate-900 placeholder:text-slate-400 placeholder:font-normal placeholder:font-sans ${
            error
