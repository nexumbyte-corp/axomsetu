...
export const buildStaffAdvanceTemplate = (data = {}, settings = {}) => {
        ...
        [
          {
            ...
            margin: [8, 6, 8, 6],
            stack: [
              { text: 'EMPLOYEE DETAILS', fontSize: 9.5, bold: true, color: '#475569', margin: [0, 0, 0, 3] },
              { text: data.staffName, fontSize: 11.5, bold: true, color: '#0f172a' },
              { text: `Employee Code: ${data.employeeId}`, fontSize: 9.5, color: '#334155' },
              { text: `Designation: ${data.designation} (${data.department})`, fontSize: 9.5, color: '#334155' },
            ],
          },
          {
            fillColor: '#f8fafc',
            ...
            margin: [8, 6, 8, 6],
            stack: [
              { text: 'ADVANCE ACCOUNT SUMMARY', fontSize: 9.5, bold: true, color: '#475569', margin: [0, 0, 0, 3] },
              { text: `Total Advance Issued: ${data.totalAdvanceGivenFormatted}`, fontSize: 9.5, color: '#0f172a' },
              { text: `Total Recovered to Date: ${data.totalRecoveredFormatted}`, fontSize: 9.5, color: '#15803d' },
              { text: `Outstanding Balance: ${data.pendingRecoveryFormatted}`, fontSize: 10.5, bold: true, color: data.pendingRecovery > 0 ? '#b91c1c' : '#15803d' },
            ],
          },
        ],
      ],
  ...
  const tableRows = [
    [
      ...
      { text: 'Type / Purpose', fontSize: 9.5, bold: true, fillColor: '#f1f5f9', color: '#0f172a' },
      { text: 'Issued (₹)', fontSize: 9.5, bold: true, alignment: 'right', fillColor: '#f1f5f9', color: '#0f172a' },
      { text: 'Recovered (₹)', fontSize: 9.5, bold: true, alignment: 'right', fillColor: '#f1f5f9', color: '#0f172a' },
      { text: 'Balance (₹)', fontSize: 9.5, bold: true, alignment: 'right', fillColor: '#f1f5f9', color: '#0f172a' },
  ...
  if (data.items.length === 0) {
    tableRows.push([
      { text: 'No advance transactions recorded.', colSpan: 6, alignment: 'center', fontSize: 8.5, color: '#64748b' },
      {}, {}, {}, {}, {},
    ]);
  } else {
    data.items.forEach((item) => {
      tableRows.push([
        ...
        { text: `${item.type.replace(/_/g, ' ')}: ${item.description}`, fontSize: 9, bold: true, color: '#0f172a' },
        { text: formatDocCurrency(item.amount), fontSize: 9, alignment: 'right', color: '#b91c1c' },
        { text: formatDocCurrency(item.recoveredAmount), fontSize: 9, alignment: 'right', color: '#15803d' },
        { text: formatDocCurrency(item.pendingAmount), fontSize: 9, alignment: 'right', bold: true, color: item.pendingAmount > 0 ? '#b91c1c' : '#15803d' },
        { text: item.status, fontSize: 8.5, alignment: 'center', bold: true, color: item.status === 'SETTLED' ? '#15803d' : '#b45309' },
      ]);
    });
  }
